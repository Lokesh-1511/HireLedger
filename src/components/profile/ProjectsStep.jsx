    ...
    <div className="col" style={{gap:'1rem'}}>
      {list.map((pr,i)=>(
        <div key={i} className="surface" style={{padding:'1rem', display:'grid', gap:'.75rem'}}>
          <label className="field-group"><span>Name</span><input required aria-required="true" aria-invalid={!!fieldErrors.projects?.[i]?.name} className={fieldErrors.projects?.[i]?.name ? 'invalid' : ''} value={pr.name} onChange={e=>updateItem(i,{name:e.target.value})} />{fieldErrors.projects?.[i]?.name && <div className="field-error-msg">{fieldErrors.projects[i].name}</div>}</label>
          ...
          <label className="field-group"><span>Link</span><input type="url" pattern="https?://.*" aria-invalid={!!fieldErrors.projects?.[i]?.link} className={fieldErrors.projects?.[i]?.link ? 'invalid' : ''} value={pr.link} onChange={e=>updateItem(i,{link:e.target.value})} placeholder="https://" />{fieldErrors.projects?.[i]?.link && <div className="field-error-msg">{fieldErrors.projects[i].link}</div>}</label>
